...
export function getRenderingOptions(settings: Partial<GameSettings> | undefined) {
  ...
  const particleCount = settings?.enableParticles ? (computedQuality === 'high' ? 1000 : computedQuality === 'medium' ? 200 : 0) : 0;
  const bloomIntensity = settings?.enableBloom ? (computedQuality === 'high' ? 1.5 : computedQuality === 'medium' ? 0.8 : 0.0) : 0.0;
