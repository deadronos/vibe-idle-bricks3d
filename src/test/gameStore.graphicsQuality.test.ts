...
describe('buildInitialState — graphicsQuality defaults', () => {
  const originalHardwareConcurrency = (
    globalThis.navigator as unknown as Navigator & { hardwareConcurrency?: number }
  ).hardwareConcurrency;
  const originalDeviceMemory = (
    globalThis.navigator as unknown as Navigator & { deviceMemory?: number }
  ).deviceMemory;
  ...
  afterEach(() => {
    // restore properties
    try {
      if (typeof originalHardwareConcurrency !== 'undefined')
        Object.defineProperty(globalThis.navigator, 'hardwareConcurrency', {
          value: originalHardwareConcurrency,
          configurable: true,
        });
      if (typeof originalDeviceMemory !== 'undefined')
